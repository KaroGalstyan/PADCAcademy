...
import { View, Text, TouchableOpacity, Alert, Modal, Image, StatusBar, StyleSheet } from 'react-native';
...
type FinishSignUpSelectScreenNavigationProp = NativeStackNavigationProp<RootStackParamList, 'FinishSignUp'>;
...
const FinishSignUp = () => {
	...
	const dateTime = data?.test?.date
		? new Date(data.test.date)
		: new Date();
	...
	const handleFinish = async () => {
		try {
			...
			setShowModal(true); // Navigacia myus ej <-
		} catch (error) {
	...
	return (
		<ScrollView className="flex-1 bg-white pt-[10px]">
			<View className='bg-[#F8F8F8] px-[20px] pt-[30px] pb-[14px]'>
				<View className="flex-row items-center mb-5 p-4 bg-[#FFFFFF]"
					style={{
						borderLeftWidth: 5, borderLeftColor: '#A19FDB', shadowColor: '#000',
					...
					<View style={{
						...
						<Text className="text-[#002531] text-[20px] font-poppinsBold">
							{initials}
						</Text>
					</View>
				...
				<View className=" p-4 mb-6 bg-[#FFFFFF]"
					style={{
						borderLeftWidth: 5, borderLeftColor: '#A19FDB', shadowColor: '#000',
						shadowOffset: { width: 0, height: 4 },
						shadowOpacity: 0.25,
					...
					<Text className="text-black text-[20px] font-inter mb-[5px]">
						{formattedDate}, {formattedTime} <Text className='text-gray-500 text-[12px]'>{timeZone}</Text>
					</Text>
					<Text className="text-gray-500 text-[14px] leading-[24px]">
						Course Type : <Text className='text-black text-[20px] leading-[24px]'> {courseType.charAt(0).toUpperCase() + courseType.slice(1)}</Text>
					</Text>
				</View>
			</View>
			<View className='px-[20px] pt-[30px]'>
				...
				<Text className="text-[14px] text-[#000] mb-6 leading-[30px]">
					Lorem ipsum dolor sit amet, consectetur adipiscing elit. Vivamus ut auctor enim. Interdum et malesuada fames ac ante ipsum primis in faucibus. Pellentesque efficitur eu ex non blandit. Sed at lorem efficitur, pellentesque urna sed, sodales nunc. Pellentesque efficitur eu ex non blandit. Sed at lorem efficitur, pellentesque urna sed, sodales nunc.
				</Text>
				...
				<TouchableOpacity onPress={handleFinish} className="rounded-xl overflow-hidden h-[50px] mb-[30px]">
					<LinearGradient
			...
			<Modal visible={showModal} transparent animationType="slide" onShow={() => StatusBar.setHidden(true)} onRequestClose={() => StatusBar.setHidden(false)}>
				<View style={StyleSheet.absoluteFill} className="bg-black/50 justify-center items-center px-5">
