...
type CourseTypeSelectScreenNavigationProp = NativeStackNavigationProp<RootStackParamList, 'CourseTypeSelect'>;
...
const CourseTypeSelect = () => {
	...
	return (
		<ScrollView className="flex-1 bg-[#F6F6F8] px-[37px] pt-10">
			<Text className="text-[30px] font-poppinsBold text-black mb-2 text-center">Online or Offline Courses</Text>
			<Text className="text-[14px] text-[#000] mb-[30px] text-center leading-[30px]">
				Learn Free or improve your skills online or offline today. Choose from a wide range of Free courses offered from top industry leaders.
			</Text>
			<View className="mb-[77px]">
				{options.map((option) => (
					<View key={option.key} className="border border-gray-300 bg-white rounded-2xl mb-[30px] px-[20px]">
						...
						<TouchableOpacity onPress={() => handleSelect(option.key as 'online' || 'offline')} className="rounded-xl overflow-hidden h-[45px] mt-4 mx-4 mb-[20px]">
							<LinearGradient
